...
export const TextArea = ({ label, required, maxLength, ...props }: TextAreaProps) => {
  ...
  return (
    <label className="flex flex-col gap-1.5">
      ...
      <textarea
        className={clsx(
          'w-full gap-2 rounded-md bg-transparent bg-white p-4 text-sm placeholder-slate-400 shadow-sm ring-1 ring-inset  invalid:bg-danger-50 focus:bg-primary-50 focus:outline-none focus:ring-2 focus:ring-primary-600 disabled:cursor-not-allowed disabled:bg-slate-50 dark:bg-slate-800 dark:placeholder-slate-300',
          tooLong
            ? 'ring-2 ring-danger-500 focus:ring-danger-500 dark:ring-danger-500'
            : 'ring-slate-500',
        )}
        {...props}
        onChange={(e) => {
